...
# Phrases that strongly indicate a real block / throttle (avoid generic marketing copy).
_RATE_LIMIT_BODY_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"\btoo many requests\b", re.I),
    re.compile(r"\brate limit(?:ed| exceeded)?\b", re.I),
    re.compile(r"\b429\b"),
    # LinkedIn-specific (login/marketing pages often contain "try again later" / "slow down".)
    re.compile(r"\bunusual activity\b", re.I),
    re.compile(r"\btemporarily restricted\b", re.I),
    re.compile(r"\btoo many sign-?in attempts\b", re.I),
    re.compile(r"\baccess (?:has been )?restricted\b", re.I),
)


def _body_suggests_rate_limit(body_lower: str) -> bool:
    return any(p.search(body_lower) for p in _RATE_LIMIT_BODY_PATTERNS)


def _is_linkedin_login_entry_url(url: str) -> bool:
    u = url.lower()
...
async def detect_rate_limit(page: Page) -> None:
    ...
    # Check for rate limit messages (avoid false positives on normal LinkedIn login/marketing copy).
    try:
        if _is_linkedin_login_entry_url(current_url):
            # Normal login flow: if the standard form is visible, skip full-body substring scan.
            try:
                if await page.locator("#username").first.is_visible(timeout=2000):
                    return
            except Exception:
                pass

        body_text = await page.locator("body").text_content(timeout=2000)
        if body_text and _body_suggests_rate_limit(body_text):
            raise RateLimitError(
                "Rate limit or access restriction detected on page.",
